...
def test_real_adapter_path_with_gateway_ollama():
    repo_root = Path(os.environ.get("TDAI_GATEWAY_CWD") or Path.cwd().parents[1]).resolve()
    env = {
        **os.environ,
        "TDAI_GATEWAY_URL": os.environ.get("TDAI_GATEWAY_URL") or "http://127.0.0.1:8420",
        "TDAI_GATEWAY_AUTO_START": os.environ.get("TDAI_GATEWAY_AUTO_START") or "1",
        "TDAI_GATEWAY_CONFIG": os.environ.get("TDAI_GATEWAY_CONFIG") or str(repo_root / "packages/tdai-memory-mcp/examples/tdai-gateway-ollama.yaml"),
        "TDAI_GATEWAY_CWD": str(repo_root),
