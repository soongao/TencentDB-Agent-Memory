...
def make_server():
    ...
    return McpServer(config=config, client=client, supervisor=supervisor), client, supervisor
...
def test_tools_call_waits_for_supervisor_and_dispatches():
    server, client, supervisor = make_server()
    response = server.handle_message({
        "jsonrpc": "2.0",
        "id": 3,
        "method": "tools/call",
...
def test_serve_handles_framed_tools_list():
    server, _, supervisor = make_server()
