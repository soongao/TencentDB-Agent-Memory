 ...
 * 中文：L1冲突检测提示（批量模式）
 * 基于Kenty验证的原型提示(l1_conflict_detection_prompt.md)。
 * 批量比较多个新记忆与统一候选池，支持跨类型合并和多目标操作。
 */
 ...
 * 中文：使用统一候选池格式化批量冲突检测提示。
 * 格式（与原型对齐）:
 * 1. 统一候选池：所有新记忆中所有现有候选者的去重列表
 * 2. 每个新记忆：内容 + 来自池的相关候选者ID列表
 * 此方法让LLM看到全局图景并在一次处理中完成跨记忆去重。
 * @param matches - 新记忆及其候选匹配项的数组
 */
